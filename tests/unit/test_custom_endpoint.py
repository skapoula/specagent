...
class TestInvoke:
    ...
    def test_raises_400(self):
        ...
        with (
            patch("specagent.llm.custom_endpoint.requests.post", return_value=ok_resp),
...
def test_create_custom_llm_custom_temperature_and_tokens():
    ...
    llm = create_custom_llm(endpoint_url="http://test/v1", temperature=0.7, max_tokens=512)
...
def test_check_llm_endpoint_health():
    ...
    with (
        patch("specagent.llm.custom_endpoint.CustomEndpointLLM") as mock_cls,
